...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        for i in range(1, 27):
            dec_str = self.apply_shift(i)
            points_w = 0
            for wrd in dec_str.split(" "):
                if is_word(self.get_valid_words(), wrd):
                    points_w += 1
            dict_points[i] = points_w

        max_value = max(dict_points.values())

        repeats_points_value_keys = []

        for i in dict_points:
            if dict_points[i] == max_value:
                repeats_points_value_keys.append(i)
        max_x = max(repeats_points_value_keys)

        #print(repeats_points_value_keys)
        return (26-max_x, self.apply_shift(max_x))
